...
def _build_ev_configs_for_milp(
    ...
    for (
        ...
        if not enabled and not has_live_session:
            log_planner(
                "debug",
                "[milp_ev] %s EV excluded: not enabled and no live session",
                label,
            )
            continue
        ...
        effective_power_kw = max(float(pwr), session_power_kw)
        effective_capacity_kwh = float(cap)
        if has_live_session and effective_capacity_kwh <= 1e-9:
            # Capacity is irrelevant to an uncontrollable fixed session, but
            # EVConfig requires a positive physical envelope. Size a synthetic
            # envelope for the documented two-hour certainty window.
            effective_capacity_kwh = max(session_power_kw * 2.0 * eff, 0.001)
        if effective_capacity_kwh <= 1e-9 or effective_power_kw <= 1e-9:
            log_planner(
                "debug",
        ...
        initial_kwh = (soc_pct / 100.0) * effective_capacity_kwh
        target_kwh = (target_pct / 100.0) * effective_capacity_kwh
        fixed_session_only = has_live_session and (
            not enabled or not connected or not smart
        )

        # When the EV is already at or above its target SoC, normally skip it.
        # A measured session remains fixed site demand, while explicitly
        # enabled past-target charging may still consume genuine surplus PV.
        ...
        if fixed_session_only:
            ...
            if has_live_session:
                ...
                target_kwh = effective_capacity_kwh
                charge_past_target = True
        else:
            # Normal smart mode: map the user deadline to an LP slot index.
        ...
        max_dc = effective_power_kw * slot_hours * eff
        ...
        configs.append(
            EVConfig(
                ...
                capacity_kwh=round(effective_capacity_kwh, 3),
                ...
                current_session_removed_from_base=(
                    inp.live_house_consumption_w > 1e-9
                    and base_includes
